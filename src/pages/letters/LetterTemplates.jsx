...
export default function LetterTemplates() {
  ...
  const loadStudentData = async () => {
    try {
      ...
      const [students, classes, families, settings] = await Promise.all([
        getAllStudents(),
        getClasses(),
        getFamilies(),
        getSettings(),
      ...
      const merged = enrollments
        .map((enrollment) => {
          ...
          return {
            ...
            className: classes.find((c) => c.id === enrollment.classId)?.name || "",
            familyName: families.find((f) => f.id === student.familyId)?.familyName || "",
          };
        })
        .filter(Boolean);
      ...
      console.error("Error loading students for letters:", err);
    } finally {
